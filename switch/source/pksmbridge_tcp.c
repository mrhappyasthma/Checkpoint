...
/** A helper function to send a file over the PKSM Bridge piece-by-piece to avoid additional memory allocations. */
static bool sendPKSMBridgeFileToSocket(int sockfd, struct pksmBridgeFile file)
{
    uint32_t sentBytes = sendDataFromBufferToSocket(sockfd, &file.checksumSize, sizeof(file.checksumSize));
...
/** A helper function to receive file bytes over the PKSM Bridge. Callers are expected to `free()` the `outFile`. */
static uint32_t receiveFileOverPKSMBridgeFromSocket(int sockfd, uint8_t** outFile, bool (*validateChecksumFunc)(struct pksmBridgeFile file))
{
    uint32_t checksumSize;
    uint32_t bytesRead = receiveDataFromSocketIntoBuffer(sockfd, &checksumSize, sizeof(checksumSize));
    ...
    // Construct a temporary `struct pksmBridgeFile` pointing to the corresponding buffers to pass to the validation function.
    struct pksmBridgeFile file = {.checksumSize = checksumSize, .checksum = checksumBuffer, .size = fileSize, .contents = fileBuffer};
...
enum pksmBridgeError sendFileOverPKSMBridgeViaTCP(uint16_t port, struct in_addr address, struct pksmBridgeFile file)
{
    int fd = createSocket();
    if (fd < 0) {
    ...
    struct pksmBridgeResponse response = createPKSMBridgeResponseForRequest(request, &checkSupportedPKSMBridgeProtocolVersionForTCP);
...
enum pksmBridgeError receiveFileOverPKSMBridgeViaTCP(
    uint16_t port, struct in_addr* outAddress, uint8_t** outFile, uint32_t* outFileSize, bool (*validateChecksumFunc)(struct pksmBridgeFile file))
{
    int fd = createSocket();
    if (fd < 0) {
        return PKSM_BRIDGE_ERROR_CONNECTION_ERROR;
    ...
    uint32_t fileSize   = receiveFileOverPKSMBridgeFromSocket(fdconn, &fileBuffer, validateChecksumFunc);
    close(fdconn);
